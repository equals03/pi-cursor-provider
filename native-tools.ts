...
/** Pending MCP or translated native exec — see proxy bridge state. */
export interface PendingExec {
  execId: string;
  execMsgId: number;
  toolCallId: string;
  toolName: string;
  /** JSON arguments string for OpenAI `tool_calls`. */
  decodedArgs: string;
...
/** Connect-style frame used by the h2 bridge (matches {@link ../proxy.ts}). */
...
function buildNativeRewrite(
  ...
  if (execCase === "readArgs") {
    const toolName = pickTool(availableToolNames, ["read", "mcp_pi_read", "read_file"]);
    if (!toolName) return null;
    const decodedArgs: Record<string, unknown> = { path: String(args.path ?? "") };
  ...
  if (execCase === "writeArgs") {
    ...
    const writeTool = pickTool(availableToolNames, ["write", "apply_patch", "search_replace", "mcp_pi_write", "mcp_pi_edit"]);
    if (!writeTool) return null;
    return {
      toolCallId,
      toolName: writeTool,
      decodedArgs: JSON.stringify({ path, content }),
  ...
  if (execCase === "deleteArgs") {
    const directDeleteTool = pickTool(availableToolNames, ["delete", "mcp_pi_delete"]);
    const path = String(args.path ?? "");
    if (directDeleteTool) {
      return {
        toolCallId,
    ...
    const bashTool = pickTool(availableToolNames, ["bash", "run_terminal_cmd", "shell", "mcp_pi_bash"]);
  ...
  if (execCase === "fetchArgs") {
    const toolName = pickTool(availableToolNames, ["webfetch", "fetch", "web_fetch", "fetch_content", "mcp_pi_fetch_content"]);
    if (!toolName) return null;
    return {
      toolCallId,
      toolName,
  ...
  if (execCase === "shellArgs" || execCase === "shellStreamArgs") {
    const toolName = pickTool(availableToolNames, ["bash", "run_terminal_cmd", "shell", "mcp_pi_bash"]);
    if (!toolName) return null;
  ...
  if (execCase === "lsArgs") {
    const toolName = pickTool(availableToolNames, ["glob", "glob_file_search", "mcp_pi_glob"]);
    if (!toolName) return null;
    return {
      toolCallId,
      toolName,
  ...
  if (execCase === "grepArgs") {
    ...
    if (!args.pattern && args.glob) {
      const globTool = pickTool(availableToolNames, ["glob", "glob_file_search", "mcp_pi_glob"]);
    ...
    const grepTool = pickTool(availableToolNames, ["grep", "rg", "codebase_search", "mcp_pi_grep", "mcp_pi_code_search"]);
